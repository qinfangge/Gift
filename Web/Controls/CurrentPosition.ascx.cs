        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            foreach (DataRow row in ArticleTable.Rows)
            {
                ...
                if(row["id"].ToString()!="1")//如果是新闻相关，则跳过去
                {
                    ...
                    {
                        nav += string.Format(" &gt;&gt; <a href='{0}List.aspx?category={1}'>{2}</a>", ArticleTableName, row["id"].ToString(), row["name"]);
                    }
                    else
                    {
                        nav += string.Format(" &gt;&gt; <a href='{0}List.aspx?category={1}&pid={3}'>{2}</a>", ArticleTableName, row["id"].ToString(), row["name"], row["pid"]);
                    }
                }
            }
