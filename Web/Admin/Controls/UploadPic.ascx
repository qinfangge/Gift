 ...
 <style>
    ...
    <script type="text/javascript">
        ...
        function setTitleImage() {
            ...
            $("#<%=this.Parent.FindControl("TitleImage").ClientID%>").val(ids);
        }

        function initPreViewImage(id) {
            var b = new Base64();
            var src = "/pic/" + b.encode("100|100|" + id) + ".jpg";
            $("#picPreView tr:eq(0)").append('<td> <img id="' + id + '" class="close" src="/Admin/js/libs/uploadify/uploadify-cancel.png"/><img class="prevView" src="' + src + '"/></td>');
        }
        $(function () {

            var ids = $("#<%=this.Parent.FindControl("TitleImage").ClientID%>").val();
            if (ids != "") {
