...
namespace CMS.DAL
{
	...
	public partial class Account
	{
		...
		public CMS.Model.Account GetModel(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 id,name,account from Account ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public CMS.Model.Account DataRowToModel(DataRow row)
		{
			CMS.Model.Account model=new CMS.Model.Account();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select id,name,account ");
